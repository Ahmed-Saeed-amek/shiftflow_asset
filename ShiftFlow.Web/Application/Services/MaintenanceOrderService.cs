...
public class MaintenanceOrderService : IMaintenanceOrderService
{
    ...
    private Task<bool> HasOtherOpenWorkAsync(int assetId, int excludeMaintenanceOrderId) =>
        AssetWorkState.HasOtherOpenWorkAsync(_db, assetId, excludeMaintenanceOrderId: excludeMaintenanceOrderId);
    ...
    public async Task<MaintenanceOrder> CreateAsync(int assetId, string? assignedToUserId, int? assignedToGroupId, string? description, DateTime? dueDate, string createdByUserId, int? orderTypeId = null, int? sourceRecurringOrderId = null, DateTime? scheduledDate = null)
    {
        ...
        await AssignNumberAsync(order, orderTypeId);
    ...
    /// <summary>Maintenance orders now honour their OrderType's own Prefix, like inspection orders
    /// do, falling back to "MO" for legacy rows with no type. The counter itself comes from the
    /// shared, atomically-claimed OrderNumberSequence (see OrderNumberGenerator).</summary>
    private async Task AssignNumberAsync(MaintenanceOrder order, int? orderTypeId)
    {
        var prefix = orderTypeId == null ? null
            : await _db.OrderTypes.Where(t => t.Id == orderTypeId).Select(t => t.Prefix).FirstOrDefaultAsync();
        if (string.IsNullOrWhiteSpace(prefix)) prefix = OrderNumberPrefixes.Maintenance;
        order.OrderNumber = await OrderNumberGenerator.NextAsync(_db, prefix, order.CreatedDate.Year);
    }
    ...
    public async Task<MaintenanceOrder> CompleteAsync(int orderId, DateTime? completedDate, List<(int SparePartId, int Quantity)> parts, string employeeUserId)
    {
        ...
        if (order.Status != OrderStatuses.Open) throw new InvalidOperationException("This maintenance order isn't awaiting a fix.");
        ...
        // so two concurrent Completes can't both decrement stock for their own parts list.
        var claimed = await _db.MaintenanceOrders.Where(m => m.Id == orderId && m.Status == OrderStatuses.Open)
        ...
        // Re-submitting a fix replaces the previous parts list - return the superseded rows'
        // quantities to stock inside this same transaction, or those units are lost forever.
        foreach (var old in order.Parts.Where(p => p.SparePartId != null))
        ...
        if (!await HasOtherOpenWorkAsync(order.AssetId, order.Id)) await SetAssetStatusAsync(order.AssetId, AssetStatuses.Working);
        await _db.SaveChangesAsync();
        await tx.CommitAsync();
        await _audit.LogAsync("Complete", "MaintenanceOrder", order.Id.ToString(), employeeUserId, oldValue: OrderStatuses.Open, newValue: order.Status);
        return order;
    }
    ...
    public async Task ApproveAsync(int orderId, string managerUserId)
    {
        ...
        var claimed = await _db.MaintenanceOrders.Where(m => m.Id == orderId && m.Status == OrderStatuses.PendingApproval)
            .ExecuteUpdateAsync(s => s.SetProperty(m => m.Status, OrderStatuses.Done).SetProperty(m => m.ClosedDate, closedDate));
        if (claimed == 0) throw new InvalidOperationException("This order isn't awaiting approval.");
        await _audit.LogAsync("Approve", "MaintenanceOrder", order.Id.ToString(), managerUserId, oldValue: OrderStatuses.PendingApproval, newValue: OrderStatuses.Done);
    }

    /// <summary>The way back out of PendingApproval: a manager sends the order to the employee to
    /// redo instead of approving it. Clears the reported cost and parts (returning their stock) and
    /// puts the order back to Open - previously PendingApproval was a dead end with no reject path.</summary>
    public async Task RejectApprovalAsync(int orderId, string? reason, string managerUserId)
    {
        ...
        var claimed = await _db.MaintenanceOrders.Where(m => m.Id == orderId && m.Status == OrderStatuses.PendingApproval)
            .ExecuteUpdateAsync(s => s
                .SetProperty(m => m.Status, OrderStatuses.Open)
                .SetProperty(m => m.ClosedDate, (DateTime?)null)
                .SetProperty(m => m.CompletedDate, (DateTime?)null)
                .SetProperty(m => m.Cost, (decimal?)null));
        if (claimed == 0) throw new InvalidOperationException("This order isn't awaiting approval.");

        foreach (var part in order.Parts.Where(p => p.SparePartId != null))
            await _spareParts.IncrementStockAsync(part.SparePartId!.Value, part.Quantity);
        _db.MaintenanceOrderParts.RemoveRange(order.Parts);
        await SetAssetStatusAsync(order.AssetId, AssetStatuses.Maintenance);
        await _db.SaveChangesAsync();
        await tx.CommitAsync();
        await _audit.LogAsync("RejectApproval", "MaintenanceOrder", order.Id.ToString(), managerUserId,
            oldValue: OrderStatuses.PendingApproval, newValue: OrderStatuses.Open, details: reason);
    }
    ...
    public async Task CancelAsync(int orderId, string? reason, string userId)
    {
        ...
        // PendingApproval is cancellable too, matching inspection orders - otherwise an order
        // awaiting sign-off could only ever go forward.
        if (order.Status is not (OrderStatuses.Open or OrderStatuses.PendingApproval))
            throw new InvalidOperationException("Only an open or pending-approval maintenance order can be cancelled.");

        // Claim the transition atomically - a manager's Cancel and the assignee's Complete could
        // otherwise both pass the in-memory status check above, last writer silently winning.
        var oldStatus = order.Status;
        var closedDate = DateTime.UtcNow;
        await using var tx = await _db.Database.BeginTransactionAsync();
        var claimed = await _db.MaintenanceOrders
            .Where(m => m.Id == orderId && (m.Status == OrderStatuses.Open || m.Status == OrderStatuses.PendingApproval))
            .ExecuteUpdateAsync(s => s.SetProperty(m => m.Status, OrderStatuses.Cancelled).SetProperty(m => m.ClosedDate, closedDate));
        if (claimed == 0) throw new InvalidOperationException("Only an open or pending-approval maintenance order can be cancelled.");

        if (!await HasOtherOpenWorkAsync(order.AssetId, order.Id)) await SetAssetStatusAsync(order.AssetId, AssetStatuses.Working);
        await _db.SaveChangesAsync();
        await tx.CommitAsync();
        await _audit.LogAsync("Cancel", "MaintenanceOrder", order.Id.ToString(), userId, oldValue: oldStatus, newValue: OrderStatuses.Cancelled, details: reason);
    }
    ...
    public async Task ReassignAsync(int orderId, string? assignedToUserId, int? assignedToGroupId, string managerUserId)
    {
        var order = await _db.MaintenanceOrders.FindAsync(orderId) ?? throw new InvalidOperationException("Maintenance order not found.");
        await EnsureOrderInScopeAsync(order.AssetId, managerUserId);
        if (order.Status is OrderStatuses.Done or OrderStatuses.Cancelled) throw new InvalidOperationException("A closed maintenance order can't be reassigned.");
        var hasUser = !string.IsNullOrWhiteSpace(assignedToUserId);
        var hasGroup = assignedToGroupId.HasValue;
        ...
        var claimed = await _db.MaintenanceOrders.Where(m => m.Id == orderId && m.Status != OrderStatuses.Done && m.Status != OrderStatuses.Cancelled)
            .ExecuteUpdateAsync(s => s
    ...
    public async Task<MaintenanceOrder?> GetByIdAsync(int id) =>
        await _db.MaintenanceOrders.AsNoTracking()
    ...
    {
        var query = _db.MaintenanceOrders.AsNoTracking()
            .Include(m => m.Asset)
            .Include(m => m.AssignedToUser)
            .Include(m => m.AssignedToGroup)
